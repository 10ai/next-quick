...
import { nextjscliRcExists } from 'utils/read-config-file.js';
import checkIfAnyDirectoryExists from 'utils/check-directory.js';
import colors from 'picocolors';

program.name('nextjs-cli').description('CLI to initialize and set up volunteer management systems');

const { green } = colors;
// Only show init command if .nextjsclirc does not exist
if (!nextjscliRcExists()) {
    ...
    // Only show rest of commands if .nextjsclirc exists
